...
# Exact live Crowdwork API payloads captured September 24, 2026. Expected casts
# were counted from the labeled source lists, not generated by the parser.
def _audited_show(name):
    import json
    from pathlib import Path
    fixture = Path(__file__).parent / "fixtures" / "lineup_boundaries.json"
    data = json.loads(fixture.read_text())
    return next(show for show in data["shows"] if show["name"] == name)


def test_cast_list_stops_before_press_paragraph():
    ...
    names = extract_lineup_names(show["description"]["body"], title=show["name"])
    assert names == show["expected_cast_deduped"]
    assert len(names) == 63
    assert "fae lily" in names
    assert "The New York Times" not in names
    assert "comedian Emily Winter" not in names


def test_cast_list_stops_before_music_references():
    show = _audited_show("This Year in Music")
    performances = extract_performances(show, rails_to_iana=RAILS_TO_IANA)
    dated_cast = next(p for p in performances if p.date_str.startswith("2026-10-14"))
    assert dated_cast.lineup_names == show["expected_cast_deduped"]
    assert len(dated_cast.lineup_names) == 24
    assert "Amy Winehouse" not in dated_cast.lineup_names
    assert "Radiohead" not in dated_cast.lineup_names


@pytest.mark.parametrize("separator", ["<br><br>", "</div><div><br></div><div>"])
def test_cast_boundary_blocks_inline_phrases_in_later_prose(separator):
    from laughtrack.scrapers.implementations.api.crowdwork.utils import extract_lineup_names
    # Vary the boundary of the fetched list: later prose must not be rescanned
    # for an inline "with" or "featuring" phrase after the cast has ended.
    show = _audited_show("One Liner Madness")
    body = show["description"]["body"]
    body = body.split("</div><div><br></div><div>", 1)[0]
    body += separator + "Music with Amy Winehouse and Radiohead.</div>"
