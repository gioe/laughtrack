...
@pytest.mark.asyncio
async def test_vb_funny_bone_collect_targets_skips_etix_discovery(monkeypatch):
    ...
    async def fail_fetch_etix_html(url: str):
        raise AssertionError(f"VB Funny Bone should not fetch blocked Etix discovery URL: {url}")
