"""
Smoke tests for Comedy on Collins using EventbriteScraper.

Verifies that the EventbriteScraper pipeline works correctly for Comedy on Collins'
Eventbrite organizer ID (109625487101).
...
def _club() -> Club:
    _c = Club(id=999, name='Comedy on Collins', address='6752 Collins Avenue', website='https://www.comedyoncollins.com', popularity=0, zip_code='33141', phone_number='', visible=True, timezone='America/New_York')
    _c.active_scraping_source = ScrapingSource(id=1, club_id=_c.id, platform='eventbrite', scraper_key='eventbrite', source_url=f'https://www.eventbrite.com/o/comedy-on-collins-{ORGANIZER_ID}', external_id=ORGANIZER_ID)
    _c.scraping_sources = [_c.active_scraping_source]
    return _c


def _make_eventbrite_event(
    name: str = "Play in My Box Comedy Show",
    ...
    return EventbriteEvent(
        ...
        description="Weekly stand-up comedy showcase at Comedy on Collins in Miami Beach.",
        location_name="The Sandbox Miami",
        location_address="6752 Collins Avenue, Miami Beach, FL",
    )
...
async def test_collect_scraping_targets_returns_organizer_id():
    """collect_scraping_targets() returns Comedy on Collins' organizer ID."""
