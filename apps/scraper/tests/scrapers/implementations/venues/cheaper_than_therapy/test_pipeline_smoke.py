"""
Smoke tests for Cheaper Than Therapy (San Francisco) using EventbriteScraper.

Verifies that the EventbriteScraper pipeline works correctly for Cheaper Than
Therapy's Eventbrite organizer ID (7381983683).
...
ORGANIZER_ID = "7381983683"
EVENT_URL = "https://www.eventbrite.com/e/cheaper-than-therapy-comedy-show-tickets-1234567890"


def _club() -> Club:
    _c = Club(id=999, name='Cheaper Than Therapy', address='533 Sutter St', website='https://cttcomedy.com', popularity=0, zip_code='94102', phone_number='', visible=True, timezone='America/Los_Angeles')
    _c.active_scraping_source = ScrapingSource(id=1, club_id=_c.id, platform='eventbrite', scraper_key='eventbrite', source_url=f'https://www.eventbrite.com/o/cheaper-than-therapy-{ORGANIZER_ID}', external_id=ORGANIZER_ID)
    _c.scraping_sources = [_c.active_scraping_source]
    return _c


def _make_eventbrite_event(
    name: str = "Cheaper Than Therapy Comedy Show",
    start_date: str = "2026-04-10T19:45:00Z",
) -> EventbriteEvent:
    return EventbriteEvent(
        name=name,
        event_url=EVENT_URL,
        start_date=start_date,
        description="Stand-up comedy at Shelton Theater in San Francisco.",
        location_name="Shelton Theater",
        location_address="533 Sutter St, San Francisco, CA",
    )
...
async def test_collect_scraping_targets_returns_organizer_id():
    """collect_scraping_targets() returns Cheaper Than Therapy's organizer ID."""
...
def test_transformation_pipeline_sets_show_date():
    ...
    scraper = EventbriteScraper(_club())
    event = _make_eventbrite_event(start_date="2026-04-10T19:45:00Z")
    page_data = EventbriteVenueData(event_list=[event])
