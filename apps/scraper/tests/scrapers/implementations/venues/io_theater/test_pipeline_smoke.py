...
import laughtrack.scrapers.implementations.venues.io_theater.scraper as _scraper_mod
...
def _club() -> Club:
    _c = Club(id=999, name='iO Theater', address='1501 N Kingsbury St', website='https://ioimprov.com', popularity=0, zip_code='60642', phone_number='', visible=True, timezone='America/Chicago')
...
async def test_get_data_retries_on_transient_5xx_then_succeeds(monkeypatch):
    ...
    returns IOTheaterPageData when the retry succeeds.
    """
    scraper = IOTheaterScraper(_club())
    call_count = {"n": 0}
    ...
    async def fake_fetch_flaky(self, url: str, **kwargs) -> dict:
        ...
        return _api_response([_show_entry()])

    async def _noop_sleep(_delay):
        pass

    monkeypatch.setattr(IOTheaterScraper, "fetch_json", fake_fetch_flaky)
    monkeypatch.setattr(_scraper_mod.asyncio, "sleep", _noop_sleep)

    result = await scraper.get_data(API_URL)

    assert isinstance(result, IOTheaterPageData), "get_data() should succeed after a transient 5xx retry"
...
async def test_get_data_returns_none_after_all_retries_exhausted(monkeypatch):
    ...
    monkeypatch.setattr(IOTheaterScraper, "fetch_json", fake_fetch_always_500)
    monkeypatch.setattr(_scraper_mod.asyncio, "sleep", _noop_sleep)
...
async def test_get_data_does_not_retry_on_4xx(monkeypatch):
    ...
    monkeypatch.setattr(IOTheaterScraper, "fetch_json", fake_fetch_404)
