"""
Pipeline smoke tests for Just the Funny (Ticketmaster / live_nation scraper).

Exercises TicketmasterScraper.get_data() → TicketmasterPageData →
transformation_pipeline.transform() using Just the Funny (venue ID KovZpZAFJEvA)
...
EVENT_URL = "https://www.ticketweb.com/event/FAKE0000JTFNY001"
...
def _make_api_event(
    name: str = "Friday Night Live - Improv Comedy Miami Show",
    date: str = "2026-04-10",
) -> dict:
    """Minimal Ticketmaster Discovery API event dict for Just the Funny."""
    return {
        "id": "FAKE0000JTFNY001",
        "name": name,
        "url": EVENT_URL,
        ...
        "priceRanges": [{"type": "standard", "min": 20.0, "max": 35.0}],
        "_embedded": {
            "venues": [{"id": VENUE_ID, "name": "Just the Funny"}],
...
def _fake_show(name: str = "Friday Night Live - Improv Comedy Miami Show") -> Show:
...
def test_transformation_pipeline_preserves_event_name():
    ...
    scraper = TicketmasterScraper(_club())
    event = _make_api_event(name="The BIG Show Improv Comedy Miami")
    page_data = TicketmasterPageData(event_list=[event])

    mock_client = MagicMock()
    mock_client.create_show.return_value = _fake_show(name="The BIG Show Improv Comedy Miami")
    with patch(
        "laughtrack.scrapers.implementations.api.ticketmaster.transformer.TicketmasterClient",
        return_value=mock_client,
    ):
    ...
    assert len(shows) == 1
    assert shows[0].name == "The BIG Show Improv Comedy Miami"
