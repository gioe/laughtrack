"""
Smoke tests for Big Couch New Orleans using EventbriteScraper.

Verifies that the EventbriteScraper pipeline works correctly for Big Couch
New Orleans's Eventbrite organizer ID (29220617465).
...
ORGANIZER_ID = "29220617465"
EVENT_URL = "https://www.eventbrite.com/e/big-couch-comedy-show-tickets-1234567890"


def _club() -> Club:
    _c = Club(id=999, name='Big Couch New Orleans', address='1045 Desire Street', website='https://bigcouchnola.com', popularity=0, zip_code='70117', phone_number='', visible=True, timezone='America/Chicago')
    _c.active_scraping_source = ScrapingSource(id=1, club_id=_c.id, platform='eventbrite', scraper_key='eventbrite', source_url=f'https://www.eventbrite.com/o/big-couch-{ORGANIZER_ID}', external_id=ORGANIZER_ID)
    _c.scraping_sources = [_c.active_scraping_source]
    return _c


def _make_eventbrite_event(
    name: str = "Big Couch Comedy Show",
    start_date: str = "2026-05-10T20:00:00Z",
) -> EventbriteEvent:
    return EventbriteEvent(
        name=name,
        event_url=EVENT_URL,
        start_date=start_date,
        description="Improv and sketch comedy at Big Couch New Orleans.",
        location_name="Big Couch New Orleans",
        location_address="1045 Desire Street, New Orleans, LA 70117",
    )
...
async def test_collect_scraping_targets_returns_organizer_id():
    """collect_scraping_targets() returns Big Couch New Orleans's organizer ID."""
...
def test_transformation_pipeline_sets_show_date():
    ...
    scraper = EventbriteScraper(_club())
    event = _make_eventbrite_event(start_date="2026-05-10T20:00:00Z")
    page_data = EventbriteVenueData(event_list=[event])
