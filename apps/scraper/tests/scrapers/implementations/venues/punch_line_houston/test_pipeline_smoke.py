"""
Pipeline smoke tests for Punch Line Comedy Club Houston (Ticketmaster / live_nation scraper).

Exercises TicketmasterScraper.get_data() → TicketmasterPageData →
transformation_pipeline.transform() using Punch Line Comedy Club Houston
(venue ID KovZ917ARGO) as the representative club.
...
EVENT_URL = "https://www.ticketmaster.com/event/FAKE0000PLHTX001"
...
def _make_api_event(
    ...
    """Minimal Ticketmaster Discovery API event dict for Punch Line Comedy Club Houston."""
    return {
        "id": "FAKE0000PLHTX001",
        "name": name,
        "url": EVENT_URL,
        ...
        "_embedded": {
            "venues": [{"id": VENUE_ID, "name": "Punch Line Houston"}],
...
def _fake_show(name: str = "Jimmy Dore") -> Show:
