"""
Smoke tests for The Riot Comedy Club Houston using EventbriteScraper.

Verifies that the EventbriteScraper pipeline works correctly for The Riot
Comedy Club's Eventbrite organizer ID (29979960920).
...
ORGANIZER_ID = "29979960920"
EVENT_URL = "https://www.eventbrite.com/e/saturday-night-standup-comedy-showcase-tickets-1234567890"


def _club() -> Club:
    _c = Club(id=999, name='The Riot Comedy Club', address='2010 Waugh Drive', website='https://theriothtx.com', popularity=0, zip_code='77006', phone_number='', visible=True, timezone='America/Chicago')
    _c.active_scraping_source = ScrapingSource(id=1, club_id=_c.id, platform='eventbrite', scraper_key='eventbrite', source_url=f'https://www.eventbrite.com/o/the-riot-comedy-club-{ORGANIZER_ID}', external_id=ORGANIZER_ID)
    _c.scraping_sources = [_c.active_scraping_source]
    return _c


def _make_eventbrite_event(
    name: str = "Saturday Night Standup Comedy Showcase",
    start_date: str = "2026-04-11T21:00:00Z",
) -> EventbriteEvent:
    return EventbriteEvent(
        name=name,
        event_url=EVENT_URL,
        start_date=start_date,
        description="Stand-up comedy showcase at The Riot Comedy Club in Houston.",
        location_name="The Riot Comedy Club",
        location_address="2010 Waugh Drive, Houston, TX",
    )
...
async def test_collect_scraping_targets_returns_organizer_id():
    """collect_scraping_targets() returns The Riot Comedy Club's organizer ID."""
...
def test_transformation_pipeline_sets_show_date():
    ...
    scraper = EventbriteScraper(_club())
    event = _make_eventbrite_event(start_date="2026-04-11T21:00:00Z")
    page_data = EventbriteVenueData(event_list=[event])
