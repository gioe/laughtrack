"""
Pipeline smoke tests for Punch Line Comedy Club - San Francisco (Ticketmaster / live_nation scraper).

Exercises TicketmasterScraper.get_data() → TicketmasterPageData →
transformation_pipeline.transform() using Punch Line SF (venue ID KovZpZAE6e7A)
...
def _make_api_event(
    ...
    return {
        ...
        "priceRanges": [{"type": "standard", "min": 25.0, "max": 40.0}],
        "_embedded": {
            "venues": [{"id": VENUE_ID, "name": "Punch Line Comedy Club - San Francisco"}],
...
def _fake_show(name: str = "Ivan Decker") -> Show:
    """Minimal Show for use in transformer mocks."""
    return Show(
        name=name,
        club_id=999,
        date=datetime(2026, 4, 5, 20, 0, tzinfo=timezone.utc),
...
def test_transformation_pipeline_preserves_event_name():
    ...
    scraper = TicketmasterScraper(_club())
    event = _make_api_event(name="W. Kamau Bell")
    page_data = TicketmasterPageData(event_list=[event])

    mock_client = MagicMock()
    mock_client.create_show.return_value = _fake_show(name="W. Kamau Bell")
    with patch(
        "laughtrack.scrapers.implementations.api.ticketmaster.transformer.TicketmasterClient",
        return_value=mock_client,
    ):
    ...
    assert len(shows) == 1
    assert shows[0].name == "W. Kamau Bell"
