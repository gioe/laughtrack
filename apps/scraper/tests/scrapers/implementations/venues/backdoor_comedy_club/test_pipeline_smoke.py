"""
Smoke tests for Backdoor Comedy Club (Richardson, TX) using EventbriteScraper.

Verifies that the EventbriteScraper pipeline works correctly for Backdoor
Comedy Club's Eventbrite organizer ID (86805735233).
...
ORGANIZER_ID = "86805735233"
EVENT_URL = "https://www.eventbrite.com/e/backdoor-comedy-club-show-tickets-1234567890"


def _club() -> Club:
    _c = Club(id=999, name='Backdoor Comedy Club', address='940 E Belt Line Rd', website='https://www.backdoorcomedy.com', popularity=0, zip_code='75081', phone_number='', visible=True, timezone='America/Chicago')
    _c.active_scraping_source = ScrapingSource(id=1, club_id=_c.id, platform='eventbrite', scraper_key='eventbrite', source_url=f'https://www.eventbrite.com/o/backdoor-comedy-club-{ORGANIZER_ID}', external_id=ORGANIZER_ID)
    _c.scraping_sources = [_c.active_scraping_source]
    return _c


def _make_eventbrite_event(
    name: str = "Backdoor Comedy Club Show",
    start_date: str = "2026-04-11T20:30:00Z",
) -> EventbriteEvent:
    return EventbriteEvent(
        name=name,
        event_url=EVENT_URL,
        start_date=start_date,
        description="Stand-up comedy in Richardson, TX.",
        location_name="Backdoor Comedy Club",
        location_address="940 E Belt Line Rd, Richardson, TX 75081",
    )
...
async def test_collect_scraping_targets_returns_organizer_id():
    """collect_scraping_targets() returns Backdoor Comedy Club's organizer ID."""
...
def test_transformation_pipeline_sets_show_date():
    ...
    scraper = EventbriteScraper(_club())
    event = _make_eventbrite_event(start_date="2026-04-11T20:30:00Z")
    page_data = EventbriteVenueData(event_list=[event])
