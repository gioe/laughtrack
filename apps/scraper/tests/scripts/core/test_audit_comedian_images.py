...
def test_update_preserves_managed_avatar_when_legacy_cdn_image_is_missing(
    ...
    read_cursor = _Cursor(
        rows=[
            ("Joe Rogan", False, True),
            ("Legacy Missing", True, False),
        ]
    )
    ...
    assert write_cursor.executions[0][1] == ("Joe Rogan",)
    ...
    assert write_cursor.executions[1][1] == ("Legacy Missing",)


def test_update_does_not_treat_inactive_managed_avatar_as_renderable(monkeypatch):
    read_cursor = _Cursor(rows=[("Inactive Asset", True, False)])
    write_cursor = _Cursor()
    monkeypatch.setattr(
        audit_comedian_images, "get_connection", lambda: _connection(read_cursor)
    )
    ...
    assert write_cursor.executions[0][1] == ("Inactive Asset",)
