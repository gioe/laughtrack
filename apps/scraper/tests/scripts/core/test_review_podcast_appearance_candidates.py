...
def test_apply_accept_resolves_alias_and_preserves_provenance(monkeypatch, tmp_path):
    conn = _FakeConn(
        ...
        comedian_rows={
            12: (12, "Ari S.", 77, False),
            77: (77, "Ari Shaffir", None, True),
        },
    )
    _install_fakes(monkeypatch, conn)
    input_path = tmp_path / "review.csv"
    ...
    assert errors == []
    assert summary.appearances_written == 1
    assert conn.appearance_writes[0][0] == 77
...
def test_apply_ignore_allows_hidden_comedian_without_resolution(monkeypatch, tmp_path):
    conn = _FakeConn(
        [_review_row(candidate_id=1)],
        comedian_rows={12: (12, "Ari Shaffir", None, False)},
    )
    _install_fakes(monkeypatch, conn)
    input_path = tmp_path / "review.csv"
    _write_decisions(input_path, [{"candidate_id": "1", "decision": "ignore"}])
...
def test_apply_accept_preflight_rejects_ineligible_batch_atomically(monkeypatch, tmp_path):
    conn = _FakeConn(
        [
            _review_row(candidate_id=1, comedian_id=12),
            _review_row(candidate_id=2, comedian_id=13, source_episode_id="ep-6"),
        ],
        comedian_rows={
            12: (12, "Ari Shaffir", None, True),
...
def test_apply_accept_dry_run_rejects_hidden_canonical_without_writes(monkeypatch, tmp_path):
    conn = _FakeConn(
        [_review_row(candidate_id=1)],
        comedian_rows={12: (12, "Ari Shaffir", None, False)},
    )
    ...
    assert summary.errored == 1
    assert summary.appearances_written == 0
    assert "hidden" in errors[0].message
    assert conn.review_updates == []
    assert conn.appearance_writes == []


def test_apply_accept_rejects_missing_parent_and_alias_cycle(monkeypatch, tmp_path):
    input_path = tmp_path / "review.csv"
    _write_decisions(input_path, [{"candidate_id": "1", "decision": "accept"}])

    missing_conn = _FakeConn(
        [_review_row(candidate_id=1)],
        comedian_rows={12: (12, "Ari Alias", 999, False)},
    )
    _install_fakes(monkeypatch, missing_conn)
    missing_summary, missing_errors = mod._apply(
        decisions_path=input_path,
        dry_run=True,
        force=False,
        reviewer="matt",
    ...
    assert missing_summary.errored == 1
    assert "not found" in missing_errors[0].message

    cycle_conn = _FakeConn(
        [_review_row(candidate_id=1)],
        comedian_rows={
            12: (12, "Ari Alias", 77, False),
            77: (77, "Ari Other Alias", 12, False),
        },
    )
    _install_fakes(monkeypatch, cycle_conn)
    cycle_summary, cycle_errors = mod._apply(
        decisions_path=input_path,
        dry_run=True,
        force=False,
        reviewer="matt",
    ...
    assert cycle_summary.errored == 1
    assert "cycle" in cycle_errors[0].message
