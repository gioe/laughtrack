#!/usr/bin/env python3
"""
Fold duplicate club 5293 "Wilson Center-NC" into canonical club 5272.

TASK-3052 found the Ticketmaster national scraper-created row is the same
Wilmington, NC venue as the Wilson Center row. All duplicate shows collide on
the database uniqueness key, so this script merges them into the surviving
canonical rows and records title mismatches such as the Daniel Tosh rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_wilson_center_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_wilson_center_duplicate_2026_06_22.py
"""
...
TASK_ID = 3160
SOURCE_AUDIT_TASK = "TASK-3052"
CANONICAL_CLUB_ID = 5272
DUPLICATE_CLUB_ID = 5293
ORIGINAL_CANONICAL_NAME = "Wilson Center, CFCC"
NORMALIZED_CANONICAL_NAME = "Wilson Center"
CANONICAL_ADDRESS = "703 North 3rd Street, Wilmington, NC 28401"
CANONICAL_WEBSITE = "https://www.wilsoncentertickets.com/"
DUPLICATE_NAME = "Wilson Center-NC"
...
CITY = "Wilmington"
STATE = "NC"
ALIAS_SOURCE = "TASK-3160"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3052-wilson-center-fold.json"
...
def _validate(cur: RealDictCursor) -> dict[str, Any]:
    clubs = _fetch_all(
        cur,
        """
        SELECT id, name, address, website, city, state, visible, status,
    ...
    elif canonical["name"] not in {ORIGINAL_CANONICAL_NAME, NORMALIZED_CANONICAL_NAME}:
        problems.append(
            f"club {CANONICAL_CLUB_ID} name is {canonical['name']!r}, "
            f"expected {ORIGINAL_CANONICAL_NAME!r} or {NORMALIZED_CANONICAL_NAME!r}"
        )
...
def _snapshot(cur: RealDictCursor) -> dict[str, Any]:
    return {
        ...
        "sources": _fetch_all(
            ...
            WHERE club_id IN (%s, %s) OR id = 4383
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3160_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3160_show_map WHERE new_show_id IS NULL",
    )
...
def _apply_future_routing(cur: RealDictCursor) -> dict[str, int]:
    counts: dict[str, int] = {}
    counts["duplicate_aliases_upserted"] = _upsert_alias(cur, DUPLICATE_NAME)
    counts["canonical_old_name_aliases_upserted"] = _upsert_alias(cur, ORIGINAL_CANONICAL_NAME)
    ...
    cur.execute(
        ...
        WHERE id = %s
        """,
        (NORMALIZED_CANONICAL_NAME, CANONICAL_ADDRESS, CANONICAL_WEBSITE, CANONICAL_CLUB_ID),
    )
    counts["canonical_clubs_normalized"] = cur.rowcount
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3160_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5293 to club 5272',
                    'task_3160_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3160_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3160_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3160_show_map m ON m.old_show_id = t.show_id
...
def _apply_club_reference_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WHERE club_id = %s
        """,
        (CANONICAL_CLUB_ID, NORMALIZED_CANONICAL_NAME, DUPLICATE_CLUB_ID),
    )
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            payload = {
                ...
                "fold": {
                    ...
                    "previous_canonical_name": ORIGINAL_CANONICAL_NAME,
                    ...
                    "aliases": [
                        ...
                        {"alias_name": ORIGINAL_CANONICAL_NAME, "city": CITY, "state": STATE},
                    ],
                    "match_key": ["date", "room"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Wilson Center duplicate club 5293 into 5272.")
