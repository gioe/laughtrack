#!/usr/bin/env python3
"""
Fold duplicate club 5356 "Tampa Theater" into club 2584.

TASK-3056 found the Ticketmaster-created spelling variant is the same Tampa, FL
venue as the canonical Tampa Theatre row. The three duplicate shows collide
exactly with existing canonical rows by date, room, and show URL, so this script
merges them into the surviving canonical rows instead of preserving both show
rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_tampa_theatre_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_tampa_theatre_duplicate_2026_06_22.py
"""
...
TASK_ID = 3056
SOURCE_AUDIT_TASK = "TASK-3056"
CANONICAL_CLUB_ID = 2584
DUPLICATE_CLUB_ID = 5356
CANONICAL_NAME = "Tampa Theatre"
DUPLICATE_NAME = "Tampa Theater"
...
CITY = "Tampa"
STATE = "FL"
ALIAS_SOURCE = "TASK-3056"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3056-tampa-theatre-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3056_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3056_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3056_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5356 to club 2584',
                    'task_3056_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3056_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3056_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3056_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 2584 (Tampa Theatre) by TASK-3056.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Tampa Theater duplicate club 5356 into 2584.")
