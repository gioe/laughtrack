#!/usr/bin/env python3
"""
Fold duplicate club 5337 "Belk Theatre" into club 5037.

TASK-3057 found the Ticketmaster-created spelling variant is the same Charlotte, NC
venue as the canonical Belk Theater row. The two duplicate shows collide
...
Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_belk_theater_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_belk_theater_duplicate_2026_06_22.py
"""
...
TASK_ID = 3057
SOURCE_AUDIT_TASK = "TASK-3057"
CANONICAL_CLUB_ID = 5037
DUPLICATE_CLUB_ID = 5337
CANONICAL_NAME = "Belk Theater"
DUPLICATE_NAME = "Belk Theatre"
...
CITY = "Charlotte"
STATE = "NC"
ALIAS_SOURCE = "TASK-3057"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3057-belk-theater-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3057_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3057_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3057_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5337 to club 5037',
                    'task_3057_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3057_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3057_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3057_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 5037 (Belk Theater) by TASK-3057.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Belk Theatre duplicate club 5337 into 5037.")
