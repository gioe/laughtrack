#!/usr/bin/env python3
"""
Fold the duplicate national-chain Funny Bone club rows created by a bulk
ticketmaster import (TASK-3321).

Background
----------
TASK-3292 (St. Louis discovery) dedup surfaced that every national-chain
Funny Bone exists twice in `clubs`:

  * an older established "<City> Funny Bone" row (ids ~300-4900) scraped via
    `etix` (the venue's own funnybone.com / etix.com ticketing) carrying
    hundreds of shows, and
  * a newer "Funny Bone Comedy Club - <City>" row (9xxx ids) created by a
    ticketmaster national-chain bulk import that re-added the chain without
    deduping against the existing rows. These carry far fewer shows.

The duplicate club shell splits shows/attribution across two rows and can
produce duplicate UI cards. This script folds each 9xxx ticketmaster
duplicate into its established etix canonical, following the audited-fold
pattern from TASK-3150
(scripts/core/fold_audited_duplicate_clubs_2026_06_21.py) and the closed-name
convention from the Stifel Theatre fold.

Confirmed pairs (canonical <- duplicate), measured 2026-06-25:

  * 323  Albany Funny Bone          <- 9485 Funny Bone Comedy Club - Albany
  * 1050 Cleveland Funny Bone       <- 9738 Funny Bone Comedy Club - Cleveland
  * 308  Columbus Funny Bone        <- 9595 Funny Bone Comedy Club - Columbus
  * 4904 Hartford Funny Bone        <- 9432 Funny Bone Comedy Club - Hartford
  * 2462 Kansas City Funny Bone     <- 9508 Funny Bone Comedy Club - Kansas City
  * 1027 Orlando Funny Bone         <- 9433 Funny Bone Comedy Club - Orlando
  * 1034 Richmond Funny Bone        <- 9321 Funny Bone Comedy Club - Richmond
  * 1028 Syracuse Funny Bone        <- 9434 Funny Bone Comedy Club - Syracuse
  * 1053 Tampa Funny Bone           <- 9255 Funny Bone Comedy Club - Tampa
  * 4504 Toledo Funny Bone          <- 9290 Funny Bone - Toledo
  * 1033 Virginia Beach Funny Bone  <- 9730 Funny Bone Comedy Club - Virginia Beach

(Hartford 9432 and Toledo 9290 are already visible=FALSE with 0 shows / 0
sources; they are included for uniform aliasing + status=closed bookkeeping.)

What this script does
---------------------
1. Validates the expected name of each canonical and duplicate row
   (FOR UPDATE lock); refuses to write on any mismatch.
2. Registers a club_aliases row routing the duplicate's "Funny Bone Comedy
   Club - <City>" name (+ city/state) to the canonical id, so a future
   ticketmaster bulk import re-resolves to canonical instead of re-inserting.
3. Moves the duplicate's scraping_sources to the canonical club, DISABLED
   (the canonical's own etix source already covers the venue; the moved
   ticketmaster source becomes a disabled fallback, never orphaned).
...
8. Writes a recovery log to docs/audits/task-3321-funny-bone-dup-folds.json.
...
-----
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_funny_bone_national_chain_dups_2026_06_25.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_funny_bone_national_chain_dups_2026_06_25.py
"""
...
TASK_ID = 3321
ALIAS_SOURCE = "TASK-3321"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3321-funny-bone-dup-folds.json"
...
FOLDS: tuple[FoldSpec, ...] = (
    FoldSpec(323, 9485, "Albany Funny Bone", "Funny Bone Comedy Club - Albany", "Albany", "NY"),
    FoldSpec(1050, 9738, "Cleveland Funny Bone", "Funny Bone Comedy Club - Cleveland", "Cleveland", "OH"),
    FoldSpec(308, 9595, "Columbus Funny Bone", "Funny Bone Comedy Club - Columbus", "Columbus", "OH"),
    FoldSpec(4904, 9432, "Hartford Funny Bone", "Funny Bone Comedy Club - Hartford", "Manchester", "CT"),
    FoldSpec(2462, 9508, "Kansas City Funny Bone", "Funny Bone Comedy Club - Kansas City", "Kansas City", "MO"),
    FoldSpec(1027, 9433, "Orlando Funny Bone", "Funny Bone Comedy Club - Orlando", "Orlando", "FL"),
    FoldSpec(1034, 9321, "Richmond Funny Bone", "Funny Bone Comedy Club - Richmond", "Richmond", "VA"),
    FoldSpec(1028, 9434, "Syracuse Funny Bone", "Funny Bone Comedy Club - Syracuse", "Syracuse", "NY"),
    FoldSpec(1053, 9255, "Tampa Funny Bone", "Funny Bone Comedy Club - Tampa", "Tampa", "FL"),
    FoldSpec(4504, 9290, "Toledo Funny Bone", "Funny Bone - Toledo", "Perrysburg", "OH"),
    FoldSpec(1033, 9730, "Virginia Beach Funny Bone", "Funny Bone Comedy Club - Virginia Beach", "Virginia Beach", "VA"),
)
...
def _create_temp_tables(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3321_folds (
    ...
    cur.execute(
        """
        CREATE TEMP TABLE task_3321_duplicate_show_map AS
        SELECT
...
def _ensure_future_routing(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WITH duplicate_sources AS (
            ...
            JOIN task_3321_folds f ON f.old_id = ss.club_id
        ),
        assigned_sources AS (
            SELECT
                id,
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3321_duplicate_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3321_duplicate_show_map m ON m.old_show_id = ts.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3321_duplicate_show_map m ON m.old_show_id = t.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        USING task_3321_duplicate_show_map m,
    ...
    cur.execute(
        ...
        FROM task_3321_duplicate_show_map m
        WHERE m.new_show_id IS NOT NULL
          AND sn.show_id = m.old_show_id
        """
    )
...
def _apply_club_reference_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3321_folds f ON f.old_id = fc.club_id
    ...
    cur.execute(
        ...
        JOIN task_3321_folds f ON f.old_id = pcv.club_id
    ...
    cur.execute(
        ...
        JOIN task_3321_folds f ON f.old_id = tc.club_id
        WHERE NOT EXISTS (
            SELECT 1
    ...
    cur.execute(
        ...
        USING task_3321_folds f, email_subscriptions existing
...
def _close_duplicate_clubs(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WHERE id IN (
            SELECT new_id FROM task_3321_folds
            UNION
            SELECT old_id FROM task_3321_folds
        )
        """
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fold duplicate national-chain Funny Bone club rows (TASK-3321)."
    )
