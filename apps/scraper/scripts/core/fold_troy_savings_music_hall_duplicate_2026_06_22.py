#!/usr/bin/env python3
"""
Fold duplicate club 5038 "Troy Savings Music Hall" into club 2579.

TASK-3059 found the Ticketmaster-created spelling variant is the same Troy, NY
venue as the canonical Troy Savings Bank Music Hall row. The duplicate show collides
with an existing canonical row by date and room. The show names and URLs differ
because the canonical row comes from the venue-owned scraper while the duplicate
row comes from Ticketmaster, so this script merges them into the surviving
canonical row instead of preserving both show rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_troy_savings_music_hall_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_troy_savings_music_hall_duplicate_2026_06_22.py
"""
...
TASK_ID = 3059
SOURCE_AUDIT_TASK = "TASK-3059"
CANONICAL_CLUB_ID = 2579
DUPLICATE_CLUB_ID = 5038
CANONICAL_NAME = "Troy Savings Bank Music Hall"
DUPLICATE_NAME = "Troy Savings Music Hall"
...
CITY = "Troy"
STATE = "NY"
ALIAS_SOURCE = "TASK-3059"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3059-troy-savings-music-hall-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3059_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3059_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3059_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5038 to club 2579',
                    'task_3059_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3059_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3059_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3059_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 2579 (Troy Savings Bank Music Hall) by TASK-3059.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Troy Savings Music Hall duplicate club 5038 into 2579.")
