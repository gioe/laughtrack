#!/usr/bin/env python3
"""
Fold duplicate club 5449 "Dr. Grins Comedy Club at The B.O.B." into club 207.

TASK-3053 found the Ticketmaster national scraper-created row is the same Grand
Rapids, MI venue as the canonical Dr. Grins Comedy Club row. The duplicate
Brian Simpson shows collide with existing canonical rows by date and room but
use the Ticketmaster title suffix " - Comedian", so this script merges them
into the surviving canonical rows instead of preserving both show rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_dr_grins_bob_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_dr_grins_bob_duplicate_2026_06_22.py
"""
...
TASK_ID = 3161
SOURCE_AUDIT_TASK = "TASK-3053"
CANONICAL_CLUB_ID = 207
DUPLICATE_CLUB_ID = 5449
CANONICAL_NAME = "Dr. Grins Comedy Club"
DUPLICATE_NAME = "Dr. Grins Comedy Club at The B.O.B."
...
CITY = "Grand Rapids"
STATE = "MI"
ALIAS_SOURCE = "TASK-3161"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3053-dr-grins-bob-fold.json"
...
def _snapshot(cur: RealDictCursor) -> dict[str, Any]:
    return {
        ...
        "sources": _fetch_all(
            ...
            WHERE club_id IN (%s, %s) OR id = 4539
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3161_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3161_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3161_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5449 to club 207',
                    'task_3161_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3161_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3161_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3161_show_map m ON m.old_show_id = t.show_id
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Dr. Grins duplicate club 5449 into 207.")
