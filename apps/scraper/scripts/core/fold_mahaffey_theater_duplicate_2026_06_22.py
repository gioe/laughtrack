#!/usr/bin/env python3
"""
Fold duplicate club 7516 "Duke Energy Center for the Arts - Mahaffey Theater" into club 2507.

TASK-3058 found the Ticketmaster-created duplicate is the same St. Petersburg,
FL venue as club 2507. The two duplicate shows collide exactly with existing
canonical rows by date, room, and show URL, so this script merges them into the
surviving canonical rows instead of preserving both show rows. Club 2507 keeps
the live Ticketmaster source, but its display name is normalized to the
official site name.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_mahaffey_theater_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_mahaffey_theater_duplicate_2026_06_22.py
"""
...
TASK_ID = 3058
SOURCE_AUDIT_TASK = "TASK-3058"
CANONICAL_CLUB_ID = 2507
DUPLICATE_CLUB_ID = 7516
OLD_CANONICAL_NAME = "Mahaffey Theater - Duke Energy Center for the Arts FL"
CANONICAL_NAME = "Duke Energy Center for the Arts - Mahaffey Theater"
DUPLICATE_NAME = "Duke Energy Center for the Arts - Mahaffey Theater"
DUPLICATE_CLOSED_NAME = (
    f"{DUPLICATE_NAME} (duplicate of club {CANONICAL_CLUB_ID}; "
    f"folded from club {DUPLICATE_CLUB_ID})"
)
CITY = "Saint Petersburg"
DUPLICATE_CITY = "St Petersburg"
STATE = "FL"
ALIAS_SOURCE = "TASK-3058"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3058-mahaffey-theater-fold.json"
ALIASES_TO_UPSERT = (
    (CANONICAL_NAME, CITY),
    (CANONICAL_NAME, DUPLICATE_CITY),
    (OLD_CANONICAL_NAME, CITY),
    ("Mahaffey Theater", DUPLICATE_CITY),
)
...
def _validate(cur: RealDictCursor) -> dict[str, Any]:
    ...
    elif canonical["name"] not in {OLD_CANONICAL_NAME, CANONICAL_NAME}:
        problems.append(
            f"club {CANONICAL_CLUB_ID} name is {canonical['name']!r}, "
            f"expected {OLD_CANONICAL_NAME!r} or {CANONICAL_NAME!r}"
        )
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3058_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3058_show_map WHERE new_show_id IS NULL",
    )
    if unmatched and unmatched["count"]:
        samples = _fetch_all(
            cur,
            """
            SELECT old_show_id, old_name, date, room
            FROM task_3058_show_map
...
def _upsert_alias(cur: RealDictCursor, alias_name: str, city: str) -> int:
    cur.execute(
        """
...
def _apply_future_routing(cur: RealDictCursor) -> dict[str, int]:
    ...
    counts["canonical_aliases_upserted"] = sum(
        _upsert_alias(cur, alias_name, city)
        for alias_name, city in ALIASES_TO_UPSERT
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3058_disposition',
                    'moved disabled duplicate source from club 7516 to club 2507',
                    'task_3058_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3058_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3058_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3058_show_map m ON m.old_show_id = t.show_id
...
def _apply_club_reference_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        "UPDATE email_subscriptions SET club_id = %s WHERE club_id = %s",
        (CANONICAL_CLUB_ID, DUPLICATE_CLUB_ID),
    )
    counts["email_subscriptions_moved"] = cur.rowcount

    cur.execute(
        "UPDATE processed_emails SET club_id = %s WHERE club_id = %s",
        (CANONICAL_CLUB_ID, DUPLICATE_CLUB_ID),
    )
    ...
    cur.execute(
        "UPDATE club_image_assets SET club_id = %s WHERE club_id = %s",
        (CANONICAL_CLUB_ID, DUPLICATE_CLUB_ID),
    )
...
def _close_duplicate(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 2507 (Duke Energy Center for the Arts - Mahaffey Theater) by TASK-3058.'
            )
        WHERE id = %s
        """,
    ...
    cur.execute(
        ...
        WHERE id = %s
          AND name = %s
        """,
        (CANONICAL_NAME, CANONICAL_CLUB_ID, OLD_CANONICAL_NAME),
    )
    counts["canonical_clubs_renamed"] = cur.rowcount
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            payload = {
                ...
                "fold": {
                    ...
                    "aliases": [
                        {"alias_name": alias_name, "city": city, "state": STATE}
                        for alias_name, city in ALIASES_TO_UPSERT
                    ],
                    "match_key": ["date", "room"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Duke Energy Center for the Arts - Mahaffey Theater duplicate club 7516 into 2507.")
