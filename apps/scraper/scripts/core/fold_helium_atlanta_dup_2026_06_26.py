#!/usr/bin/env python3
"""
Fold the duplicate Helium Atlanta club row 4734 into canonical 134 (TASK-3381).

Background
----------
TASK-3297 (Atlanta discovery) dedup surfaced that Helium Comedy Club in
Alpharetta GA exists twice in `clubs` (both carry the identical
google_place_id ChIJqZmsWwCf9YgRVCw_z__HLdQ — definitively the same venue):

  * 134  'Helium Comedy Club - Atlanta'  — CANONICAL: the venue's own
         seatengine_classic source (atlanta.heliumcomedy.com/events),
         chain_id=2 (Helium chain), website set, 522 shows / 226 upcoming —
         the real full calendar.
  * 4734 'Helium Comedy Club - GA'       — thin DUPLICATE created by the
         national ticketmaster bulk import (tm_id Z7r9jZakbo,
         ticketmaster_comedy), no website, no chain_id, 13 shows / 13 upcoming
         (9 of which collide by date with 134), 67 ticket click-events.

Same class as the TASK-3321 Funny Bone / TASK-3465 Laugh Factory national-chain
dedups. The duplicate splits shows/attribution and can produce duplicate UI
cards. This folds 4734 -> 134 using the audited-fold pattern. The alias insert
relies on the club_aliases_set_normalized trigger (TASK-3462) to populate the
normalized_* columns; that migration is live on prod.

What this script does
---------------------
1. Validates the canonical/duplicate names under FOR UPDATE; refuses on mismatch.
2. Adds a club_aliases row routing 'Helium Comedy Club - GA' (Alpharetta, GA)
   to canonical 134 (raw columns only; trigger normalizes), so a future
   ticketmaster bulk import re-resolves to 134 instead of re-inserting.
3. Moves the duplicate's ticketmaster scraping_source to canonical, DISABLED
   (134's seatengine source already covers the venue; the moved TM source
   becomes a disabled fallback, never orphaned).
...
6. Closes the duplicate: renames to
   '<dup name> (duplicate of club 134; folded from club 4734)',
   visible=FALSE, status='closed', closed_at=NOW().
7. Recomputes clubs.total_shows for both rows.
8. Writes a recovery log to docs/audits/task-3381-helium-atlanta-fold.json.
...
-----
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_helium_atlanta_dup_2026_06_26.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_helium_atlanta_dup_2026_06_26.py
"""
...
TASK_ID = 3381
ALIAS_SOURCE = "TASK-3381"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3381-helium-atlanta-fold.json"
...
FOLDS: tuple[FoldSpec, ...] = (
    FoldSpec(134, 4734, "Helium Comedy Club - Atlanta", "Helium Comedy Club - GA", "Alpharetta", "GA"),
)
...
def _create_temp_tables(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3381_folds (
    ...
    cur.execute(
        """
        CREATE TEMP TABLE task_3381_duplicate_show_map AS
        SELECT
...
def _ensure_future_routing(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WITH duplicate_sources AS (
            ...
            JOIN task_3381_folds f ON f.old_id = ss.club_id
        ),
        assigned_sources AS (
            SELECT
                id,
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3381_duplicate_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3381_duplicate_show_map m ON m.old_show_id = ts.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3381_duplicate_show_map m ON m.old_show_id = t.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        USING task_3381_duplicate_show_map m,
    ...
    cur.execute(
        ...
        FROM task_3381_duplicate_show_map m
        WHERE m.new_show_id IS NOT NULL
          AND sn.show_id = m.old_show_id
        """
    )
...
def _apply_club_reference_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3381_folds f ON f.old_id = fc.club_id
    ...
    cur.execute(
        ...
        JOIN task_3381_folds f ON f.old_id = pcv.club_id
    ...
    cur.execute(
        ...
        JOIN task_3381_folds f ON f.old_id = tc.club_id
        WHERE NOT EXISTS (
            SELECT 1
    ...
    cur.execute(
        ...
        USING task_3381_folds f, email_subscriptions existing
...
def _close_duplicate_clubs(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WHERE id IN (
            SELECT new_id FROM task_3381_folds
            UNION
            SELECT old_id FROM task_3381_folds
        )
        """
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fold the duplicate Helium Atlanta club row 4734 into canonical 134 (TASK-3381)."
    )
