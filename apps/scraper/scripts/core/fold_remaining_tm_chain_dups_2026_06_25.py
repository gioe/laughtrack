#!/usr/bin/env python3
"""
Fold the remaining ticketmaster bulk-import duplicate club rows surfaced by the
cross-chain name-normalized audit (TASK-3459).

Background
----------
TASK-3321 (Funny Bone), TASK-3457 (Comedy Zone) and TASK-3381 (Helium Atlanta)
folded the duplicate "<Chain> ... - <City>" rows that a ticketmaster
national-chain bulk import re-created on top of established venues. Those were
found by spot-checking three chains. This task ran a name-normalized detector
across all 924 ticketmaster-sourced visible clubs (comparing each against
same-(city, state) non-ticketmaster rows and requiring a shared *distinctive*
brand token, not just a shared city word) to catch every remaining pair.

Confirmed pairs (canonical <- duplicate), verified by identical street address,
measured 2026-06-25:

  * 46  Stress Factory Bridgeport                 <- 5099 Stress Factory Comedy Club - Bridgeport (Bridgeport, CT)
  * 132 Helium & Elements Restaurant - Buffalo    <- 4969 Helium Comedy Club - Buffalo          (Buffalo, NY)
  * 71  The Comedy Loft of DC                      <- 4508 DC Comedy Loft and Bier Baron Tavern  (Washington, DC)

In each case the canonical is the established low-id row scraped from the
venue's own ticketing (seatengine) with hundreds of shows and its own website;
the duplicate is the bulk-import ticketmaster row with no website and a handful
of shows. (Helium Atlanta 4734<-134 is TASK-3381; the Laugh Factory Las Vegas
172/6810 pair was deferred for manual review — reversed roles plus a venue
relocation make it unsafe to auto-fold.)

What this script does
---------------------
Same audited-fold pattern as scripts/core/fold_funny_bone_national_chain_dups_2026_06_25.py
(TASK-3321) / fold_audited_duplicate_clubs_2026_06_21.py (TASK-3150):

1. Validates each canonical/duplicate name under FOR UPDATE; refuses on mismatch.
2. Registers a club_aliases row routing the duplicate's bulk-import name (+
   city/state) to the canonical id. Per TASK-3462 the insert supplies only the
   raw alias_name/city/state — the club_aliases_set_normalized trigger derives
   normalized_alias_name/normalized_city/normalized_state via lt_normalize_alias_key.
   REQUIRES migration 20260625140000_club_aliases_normalize_trigger to be applied.
3. Moves the duplicate's scraping_sources to the canonical club, DISABLED (the
   canonical's own source already covers the venue; the moved ticketmaster
   source becomes a disabled fallback, never orphaned).
4. Re-points the duplicate's shows: shows colliding on (date, room) with an
   ...
   copied to the survivor, click-events + notifications repointed, duplicate
   show deleted); non-colliding shows are moved to canonical.
...
8. Writes a recovery log to docs/audits/task-3459-remaining-tm-chain-dup-folds.json.

Idempotent: alias upsert, source moves only affect rows still on the duplicate,
the show map only matches shows still on the duplicate, and closed rows already
carrying the closed-name are matched by it. Safe to re-run.

Usage
-----
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_remaining_tm_chain_dups_2026_06_25.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_remaining_tm_chain_dups_2026_06_25.py
"""
...
TASK_ID = 3459
ALIAS_SOURCE = "TASK-3459"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3459-remaining-tm-chain-dup-folds.json"
...
FOLDS: tuple[FoldSpec, ...] = (
    FoldSpec(46, 5099, "Stress Factory Bridgeport", "Stress Factory Comedy Club - Bridgeport", "Bridgeport", "CT"),
    FoldSpec(132, 4969, "Helium & Elements Restaurant - Buffalo", "Helium Comedy Club - Buffalo", "Buffalo", "NY"),
    FoldSpec(71, 4508, "The Comedy Loft of DC", "DC Comedy Loft and Bier Baron Tavern", "Washington", "DC"),
)
...
def _create_temp_tables(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3459_folds (
    ...
    cur.execute(
        """
        CREATE TEMP TABLE task_3459_duplicate_show_map AS
        SELECT
...
def _ensure_future_routing(cur: RealDictCursor) -> dict[str, int]:
    ...
    # the raw alias_name/city/state. ON CONFLICT still targets the trigger-maintained
    # columns (the trigger fires BEFORE the conflict check, so the dedup key is set
    # in time). REQUIRES migration 20260625140000_club_aliases_normalize_trigger.
    cur.execute(
        """
    ...
    cur.execute(
        ...
        WITH duplicate_sources AS (
            ...
            JOIN task_3459_folds f ON f.old_id = ss.club_id
        ),
        assigned_sources AS (
            SELECT
                id,
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3459_duplicate_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3459_duplicate_show_map m ON m.old_show_id = ts.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3459_duplicate_show_map m ON m.old_show_id = t.show_id
        WHERE m.new_show_id IS NOT NULL
    ...
    cur.execute(
        ...
        USING task_3459_duplicate_show_map m,
    ...
    cur.execute(
        ...
        FROM task_3459_duplicate_show_map m
        WHERE m.new_show_id IS NOT NULL
          AND sn.show_id = m.old_show_id
        """
    )
...
def _apply_club_reference_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3459_folds f ON f.old_id = fc.club_id
    ...
    cur.execute(
        ...
        JOIN task_3459_folds f ON f.old_id = pcv.club_id
    ...
    cur.execute(
        ...
        JOIN task_3459_folds f ON f.old_id = tc.club_id
        WHERE NOT EXISTS (
            SELECT 1
    ...
    cur.execute(
        ...
        USING task_3459_folds f, email_subscriptions existing
...
def _close_duplicate_clubs(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        WHERE id IN (
            SELECT new_id FROM task_3459_folds
            UNION
            SELECT old_id FROM task_3459_folds
        )
        """
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fold remaining ticketmaster bulk-import duplicate club rows (TASK-3459)."
    )
