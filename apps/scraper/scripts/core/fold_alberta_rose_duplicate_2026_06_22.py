#!/usr/bin/env python3
"""
Fold duplicate club 5359 "ALBERTA ROSE" into club 5358.

TASK-3060 found the Ticketmaster-created spelling variant is the same Portland, OR
venue as the canonical Alberta Rose Theatre row. The duplicate show collides
with an existing canonical row by date, room, name, and Ticketmaster URL, so
this script merges it into the surviving canonical row instead of preserving
both show rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_alberta_rose_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_alberta_rose_duplicate_2026_06_22.py
"""
...
TASK_ID = 3060
SOURCE_AUDIT_TASK = "TASK-3060"
CANONICAL_CLUB_ID = 5358
DUPLICATE_CLUB_ID = 5359
CANONICAL_NAME = "Alberta Rose Theatre"
DUPLICATE_NAME = "ALBERTA ROSE"
...
CITY = "Portland"
STATE = "OR"
ALIAS_SOURCE = "TASK-3060"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3060-alberta-rose-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3060_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3060_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3060_disposition',
                    'moved disabled duplicate Ticketmaster source from club 5359 to club 5358',
                    'task_3060_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3060_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3060_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3060_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 5358 (Alberta Rose Theatre) by TASK-3060.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold ALBERTA ROSE duplicate club 5359 into 5358.")
