#!/usr/bin/env python3
"""
Fold duplicate club 4509 "Uptown Theatre Providence" into club 80.

TASK-3061 found the Ticketmaster-created spelling variant is the same Providence, RI
venue as the canonical Uptown Theater row. The duplicate show collides
...
Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_uptown_theatre_providence_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_uptown_theatre_providence_duplicate_2026_06_22.py
"""
...
TASK_ID = 3061
SOURCE_AUDIT_TASK = "TASK-3061"
CANONICAL_CLUB_ID = 80
DUPLICATE_CLUB_ID = 4509
CANONICAL_NAME = "Uptown Theater"
DUPLICATE_NAME = "Uptown Theatre Providence"
...
CITY = "Providence"
STATE = "RI"
ALIAS_SOURCE = "TASK-3061"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3061-uptown-theatre-providence-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3061_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3061_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3061_disposition',
                    'moved and disabled duplicate Ticketmaster source from club 4509 to club 80',
                    'task_3061_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3061_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3061_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3061_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 80 (Uptown Theater) by TASK-3061.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Uptown Theatre Providence duplicate club 4509 into 80.")
