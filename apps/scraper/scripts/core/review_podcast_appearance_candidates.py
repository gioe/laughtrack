#!/usr/bin/env python3
"""Export and apply review decisions for podcast episode appearance candidates."""
...
_LOAD_CANDIDATES_SQL = """
    ...
    WHERE (%s::boolean OR ear.candidate_status = 'pending')
    ORDER BY ear.confidence DESC, ear.id
    {limit_clause}
"""
...
def _load_candidates(*, include_resolved: bool = False, limit: Optional[int] = None) -> list[AppearanceCandidate]:
...
def _print_apply_report(summary: ApplySummary, errors: list[ApplyError], *, dry_run: bool) -> None:
    ...
    if summary.appearances_written:
        ...
        print(f"  {verb} {summary.appearances_written} episode_appearances row(s)")


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Review podcast episode appearance candidates")
    sub = parser.add_subparsers(dest="command", required=True)

    p_export = sub.add_parser("export", help="Write pending candidates to CSV")
