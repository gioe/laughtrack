#!/usr/bin/env python3
"""
Fold duplicate club 7557 "Stifel Theatre" into club 2509.

TASK-3055 found the Ticketmaster national scraper-created row is the same
St. Louis, MO venue as the canonical Stifel Theatre - St. Louis row. The three
duplicate shows collide exactly with existing canonical rows by date, room, and
Ticketmaster URL, so this script merges them into the surviving canonical rows
instead of preserving both show rows.

Usage:
    cd apps/scraper
    make run-script SCRIPT=scripts/core/fold_stifel_theatre_duplicate_2026_06_22.py ARGS='--dry-run'
    make run-script SCRIPT=scripts/core/fold_stifel_theatre_duplicate_2026_06_22.py
"""
...
TASK_ID = 3055
SOURCE_AUDIT_TASK = "TASK-3055"
CANONICAL_CLUB_ID = 2509
DUPLICATE_CLUB_ID = 7557
CANONICAL_NAME = "Stifel Theatre - St. Louis"
DUPLICATE_NAME = "Stifel Theatre"
...
CITY = "Saint Louis"
STATE = "MO"
ALIAS_SOURCE = "TASK-3055"
RECOVERY_LOG_PATH = _root / "docs" / "audits" / "task-3055-stifel-theatre-st-louis-fold.json"
...
def _create_temp_show_map(cur: RealDictCursor) -> None:
    cur.execute(
        """
        CREATE TEMP TABLE task_3055_show_map AS
        SELECT
    ...
    unmatched = _fetch_one(
        cur,
        "SELECT COUNT(*) AS count FROM task_3055_show_map WHERE new_show_id IS NULL",
    )
    ...
    cur.execute(
        ...
        SET club_id = %s,
            ...
            metadata = COALESCE(ss.metadata, '{}'::jsonb)
                || jsonb_build_object(
                    'task_3055_disposition',
                    'moved disabled duplicate Ticketmaster source from club 7557 to club 2509',
                    'task_3055_source_audit',
                    %s
                ),
            updated_at = NOW()
...
def _apply_show_updates(cur: RealDictCursor) -> dict[str, int]:
    ...
    cur.execute(
        ...
        JOIN task_3055_show_map m ON m.old_show_id = li.show_id
    ...
    cur.execute(
        ...
        JOIN task_3055_show_map m ON m.old_show_id = ts.show_id
    ...
    cur.execute(
        ...
        FROM tickets t
        JOIN task_3055_show_map m ON m.old_show_id = t.show_id
    ...
    cur.execute(
        ...
        SET name = %s,
            ...
            description = concat_ws(
                E'\n\n',
                NULLIF(description, ''),
                'Duplicate row consolidated into club 2509 (Stifel Theatre - St. Louis) by TASK-3055.'
            )
        WHERE id = %s
        """,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fold Stifel Theatre duplicate club 7557 into 2509.")
