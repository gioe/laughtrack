...
**Class/camp drop on an all-comedy OvationTix venue (`exclude_title_patterns`):** an all-improv theater can still have its OvationTix calendar mix the public shows with improv *classes*, *workshops*, and *kids camps*. Those carry comedy keywords ("improv"), so the `comedy_filter` keyword heuristic alone keeps them. Add `metadata.exclude_title_patterns` (list of case-insensitive substrings, e.g. `["CLASSES", "Kids Camp"]`) to drop them by title. **Gotcha:** `exclude_title_patterns` is only applied when `comedy_filter` is also `true` (it runs inside the comedy filter pass), so set both even on a single-purpose comedy venue. Confirm the real shows survive the comedy keyword check before closing (Whole World Improv Theatre client 36156, TASK-3366: 84 shows across 8 improv series kept; classes + Kids Camp dropped, zero real-show loss).
