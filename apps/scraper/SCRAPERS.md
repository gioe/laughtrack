...
`TixrPublicCardScraper` parses the venue page directly and does not call `TixrClient.get_event_detail_from_url()`. Use this for St. Marks Comedy Club and House of Comedy Bloomington style Webflow cards where DataDome blocks Tixr detail enrichment but the public card has enough data to build shows. It recognizes five venue-owned card templates: the St. Marks `.event-item > a.ticket-links` Webflow cards (month/day-only date, year inferred), the Black Buzzard `.event-item` Webflow variant (`.main-title-hover-2` title + absolute `.date-2.lrg` date like `Aug 21, 2026` + `a.row-clickto-ticket` Tixr link, with per-card JSON-LD `offers.price`), The Stand NYC `.show_row` Bootstrap cards (date encoded in the show-page slug), the Phil Long Music Hall `.day-card` Webflow variant (title in `.b-show`, absolute date + time in the two `.event-info_dates p.b-venue.date` nodes like `October 23, 2026` / `8:00 pm`, optional `Featuring:` performer in `.b-venue.name`, Tixr buy link via `a[href*="tixr.com"]`), and Great Outdoors Comedy Festival `.show-card-content` Webflow cards (date/city pills, headliner/subheadliner text, Tixr buy link). For GOCF city-specific sources, point `source_url` at the city page (e.g. `https://www.greatoutdoorscomedyfestival.com/cities/halifax`) and set `metadata.gocf_city` (e.g. `"Halifax"`) as a guard; the direct Tixr event pages are DataDome-blocked. When the venue homepage already embeds full Schema.org Event JSON-LD per card, that block supplies the offer price keyed by the Tixr ticket URL. (The Black Buzzard at Oskar Blues — Denver, TASK-3384 — uses the second variant; note its `/events` page is a shared "Bandwagon" Tixr group with other venues, so point `source_url` at the homepage, which carries only the venue's own events.)
...
**Products-collection mode (TASK-3012):** Some venues sell each show as a dated store product (collection `typeName='products'`, type 13, e.g. `/tickets/p/june-19-2026`) instead of an Events collection, so `GetItemsByMonth` returns `[]`. Onboard with `scraping_url` = the collection PAGE url (e.g. `https://<domain>/tickets`, no `collectionId` param) and `metadata.collection_type='products'`; the scraper then reads the page via `?format=json`, follows `pagination.nextPageUrl`, and parses each product's show date from its `fullUrl` slug (`/tickets/p/june-19-2026`, including ordinal/yearless variants like `/tickets/p/sat-aug-8th-rhino-room-stand-up-8pm`) plus the time from the title (`@8pm`, bare `8pm`, default 19:00 if absent). Recently past yearless dates are skipped rather than rolled into next year; far-past dates can roll forward for year-boundary products. Examples: Westside Improv Studio (Wheaton, IL); Rhino Comedy (TASK-3566).

**Events-stacked page JSON mode (TASK-3566):** Some Squarespace Events collections report a nonzero `collection.itemCount` in `<events-page>?format=json`, but `/api/open/GetItemsByMonth?collectionId=<id>` returns `[]`. If the page JSON exposes event records in `upcoming` and/or `past` arrays with `structuredContent._type='CalendarEvent'` / `startDate`, onboard with `scraping_url` = the collection PAGE url and `metadata.collection_type='events_stacked_json'`. The scraper reads `<page>?format=json`, parses those event arrays, drops stale events before the current scrape time, and enriches each show from its detail page when possible. Example source shape: The Fear City Comedy Club `/shows`; on 2026-07-03 the mode parsed correctly but returned 0 future shows after stale filtering, so the venue was not onboarded yet.

**Mixed-use venues (comedy title filter, OFF by default):** a community performing-arts center often runs an occasional stand-up comedy night / open mic on the same Squarespace Events collection as films, plays, youth theatre, dance and concerts. Two opt-in `scraping_sources.metadata` title filters isolate comedy (works in events, products, and events-stacked page JSON modes; pattern parsing via the shared `BaseScraper.compile_title_patterns`, include-then-exclude loop mirrors `ticketweb`/`sellingticket`/`showare`, so existing single-purpose Squarespace venues are unchanged):
...
**Class/camp drop on an all-comedy OvationTix venue (`exclude_title_patterns`):** an all-improv theater can still have its OvationTix calendar mix the public shows with improv *classes*, *workshops*, and *kids camps*. Those carry comedy keywords ("improv"), so the `comedy_filter` keyword heuristic alone keeps them. Add `metadata.exclude_title_patterns` (list of case-insensitive regex/substrings, e.g. `["CLASSES", "Kids Camp"]`) to drop them by title. `exclude_title_patterns` is applied **independently of `comedy_filter`** (TASK-3480): it runs in its own pass before the comedy filter, so an all-comedy venue can drop classes/camps **without** setting `comedy_filter`. That matters because enabling `comedy_filter` subjects every real show to the `is_comedy_event` keyword gate — a legit show whose title/description lacks a comedy keyword would be lost. Only add `comedy_filter: true` when the calendar genuinely mixes in non-comedy programming (plays, concerts) that the exclude list can't enumerate. First onboarded coupled (Whole World Improv Theatre client 36156, TASK-3366: 84 shows across 8 improv series kept, classes + Kids Camp dropped); decoupled in TASK-3480.
