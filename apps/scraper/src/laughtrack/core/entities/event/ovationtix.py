"""
Shared OvationTix event data model.

Base dataclass for all OvationTix-backed venue events. Each venue event model
inherits from this and only overrides behavior that differs (e.g., date parsing
for Uncle Vinnie's ISO 8601 dates).
"""
...
@dataclass
class OvationTixEvent:
    """
    A single upcoming performance from the OvationTix API.

    One production (e.g. "Comedy Show") has many performances — one per
    ...
    def to_show(self, club: Club, enhanced: bool = True) -> Optional[Show]:
        ...
        try:
            ...
            Logger.error(f"Failed to transform OvationTixEvent: {e}")
            return None
