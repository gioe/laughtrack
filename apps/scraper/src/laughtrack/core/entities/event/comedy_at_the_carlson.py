...
Each event maps to one performance (date/time slot) within a production.
"""
...
@dataclass
class ComedyAtTheCarlsonEvent:
    """
    A single upcoming performance from the OvationTix API for Comedy @ The Carlson.

    One production (e.g. "T.J. Miller") has multiple performances — one per
    ...
    def to_show(self, club: Club, enhanced: bool = True) -> Optional[Show]:
        ...
        try:
            ...
            Logger.error(f"Failed to transform ComedyAtTheCarlsonEvent: {e}")
            return None
