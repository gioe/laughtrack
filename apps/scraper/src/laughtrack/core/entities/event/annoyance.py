"""
Data model for a single performance from The Annoyance Theatre Chicago.

Show data is fetched from the ThunderTix calendar API at:
  https://theannoyance.thundertix.com/reports/calendar?week=0&start={ts}&end={ts+7d}
...
@dataclass
class AnnoyancePerformance(ShowConvertible):
    """
    Data model representing a single performance from The Annoyance Theatre.
    ...
    def from_api_response(cls, data: dict, base_url: str) -> "AnnoyancePerformance":
        """Create an AnnoyancePerformance from a raw ThunderTix API dict."""
