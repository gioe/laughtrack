"""
Data model for a single event from an Etix venue page.

Etix venue pages at https://www.etix.com/ticket/v/{venue_id}/... have a
consistent HTML structure: microdata (itemprop=startDate), CSS classes
(row performance, performance-name, performance-datetime), and ticket URLs
(/ticket/p/{id}/...).  This generic model works across all Etix venues.
"""
...
@dataclass
class EtixEvent(ShowConvertible):
    """A single event scraped from an Etix venue page."""
    ...
    def _parse_start_date(self, club: Club) -> Optional[datetime]:
        ...
        tz = club.timezone or "America/New_York"
