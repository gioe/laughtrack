"""Event extraction from a The Events Calendar (Tribe) REST API response."""
...
class TribeEventExtractor:
    """Converts raw Tribe Events REST API JSON into TribeEvent objects."""

    @staticmethod
    def extract_events(api_response: Dict[str, Any]) -> List[TribeEvent]:
        """Extract TribeEvent objects from a single API response page."""
        events = []
        for raw in api_response.get("events", []):
            try:
                event = TribeEventExtractor._parse_event(raw)
                if event:
                    events.append(event)
            except Exception as e:
                Logger.warn(f"TribeEventExtractor: skipping event due to error: {e}")
        return events
    ...
    @staticmethod
    def _parse_event(raw: Dict[str, Any]) -> TribeEvent:
