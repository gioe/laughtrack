"""HTML extraction for Etix venue pages."""
...
class EtixExtractor:
    """Parses HTML from an Etix venue page."""

    @staticmethod
    def extract_events(html: str) -> List[EtixEvent]:
        ...
        for card_match in _CARD_RE.finditer(html):
            ...
            event = EtixExtractor._parse_card(card)
    ...
    @staticmethod
    def _parse_card(card_html: str) -> Optional[EtixEvent]:
        """Parse a single event card and return an EtixEvent."""
        ...
        if not start_date:
            Logger.debug(
                f"EtixExtractor: skipping card — no date for '{title}'"
            )
            return None
