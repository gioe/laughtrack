...
class StandupNYScraper(BaseScraper):
    ...
    async def get_data(self, url: str) -> Optional[JSONDict]:
        ...
        if not event:
            Logger.warn(f"{self._log_prefix}: No event found for ticket URL: {url}", self.logger_context)
            return None
    ...
    async def scrape_async(self) -> List[Show]:
        ...
        try:
            ...
            if not hasattr(self, "page_data") or not self.page_data:
                Logger.warn(f"{self._log_prefix}: No base event data available", self.logger_context)
                return []

            # Step 2: Enhance VenuePilot events in parallel if any URLs found
            if venuepilot_urls:
                Logger.info(f"{self._log_prefix}: Enhancing {len(venuepilot_urls)} VenuePilot events", self.logger_context)
                ...
                Logger.info(
                    f"{self._log_prefix}: Successfully enhanced {enhanced_count}/{len(venuepilot_urls)} VenuePilot events",
                    self.logger_context,
                )
            ...
            Logger.info(f"{self._log_prefix}: StandUp NY scraping completed. Found {len(shows)} shows", self.logger_context)
    ...
    async def cleanup(self):
        ...
        try:
            ...
            Logger.warn(f"{self._log_prefix}: Error during cleanup: {e}", self.logger_context)
