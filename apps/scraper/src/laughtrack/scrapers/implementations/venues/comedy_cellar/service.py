...
from laughtrack.utilities.infrastructure.api.base_api_client import BaseAPIClient

from .extractor import ComedyCellarExtractor


class ComedyCellarService(BaseAPIClient):
    """
    ...
    def _setup_api_config(self) -> None:
        """Setup request headers and payload templates."""
        ...
        self.shows_headers = BaseHeaders.get_headers(
            ...
            headers={
                "x-code-localize": "5e49d74493acf322b0167e9e7b872651df8a589f499047190a418f91bded7849.MTc1MDI2ODU1My0xODUuMTk5LjEwMy4xMzE=",
                "x-page-creation": "1750264947",
            },
        )

        self.lineup_payload_template = 'action=cc_get_shows&json={{"date":"{}","venue":"newyork","type":"lineup"}}'

    async def get_lineup_data(self, date_key: str) -> Optional[ComedyCellarLineupAPIResponse]:
        ...
        try:
            ...
            return ComedyCellarExtractor.extract_lineup_data(response_text)

        except Exception as e:
            self._log_error("fetching lineup", e, date_key)
            return None

    async def get_shows_data(self, date_key: str) -> Optional[ComedyCellarShowsAPIResponse]:
        ...
        try:
            ...
            return ComedyCellarExtractor.extract_shows_data(response_data)

        except Exception as e:
            self._log_error("fetching shows", e, date_key)
            return None
    ...
    async def discover_available_dates(self) -> list[str]:
        ...
        try:
            ...
            self._log_success("discovered dates", details=f"found {len(dates)} dates")
            return dates

        except Exception as e:
            self._log_error("discovering dates", e)
            return []
