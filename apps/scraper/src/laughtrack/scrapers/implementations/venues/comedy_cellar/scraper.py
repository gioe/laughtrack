...
class ComedyCellarScraper(BaseScraper):
    ...
    async def get_data(self, target: ScrapingTarget) -> Optional[ComedyCellarDateData]:
        ...
        try:
            ...
            Logger.error(f"{self._log_prefix}: Error extracting data for {target}: {str(e)}", self.logger_context)
            return None
    ...
    async def _fetch_all_raw_data(
        ...
        """
        count = len(targets)
        Logger.info(f"{self._log_prefix}: Starting sequential fetch of {count} targets (Comedy Cellar rate limit)", self.logger_context)
        ...
        Logger.info(f"{self._log_prefix}: Successfully fetched data from {successful_fetches}/{count} targets", self.logger_context)
        return results
