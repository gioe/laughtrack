"""Shared Comedy Works data extraction utilities.

Both Comedy Works Downtown and Comedy Works South share the same
comedyworks.com Rails site and identical HTML structure. This base
extractor is parameterized by the CSS class that distinguishes each
location's ticket section (e.g. 'club-downtown' vs 'club-south').
"""
...
class ComedyWorksBaseExtractor:
    """Base extractor for Comedy Works venues.

    Subclasses set ``location_css_class`` to the CSS class that identifies
    their ticket-location section (e.g. ``"club-downtown"``).
    """

    location_css_class: str = ""  # override in subclass

    @classmethod
    def extract_comedian_slugs(cls, html_content: str) -> List[str]:
        """
        Parse the events list page and return unique comedian slugs.

        Args:
            html_content: HTML from /events?downtown=1 or /events?south=1
    ...
    @classmethod
    def extract_events_from_detail(cls, html_content: str, slug: str) -> List[ComedyWorksDowntownEvent]:
        """
        Parse a comedian detail page and extract showtimes for this location.
        ...
        Returns:
            List of ComedyWorksDowntownEvent, one per showtime at this location
        """
        soup = BeautifulSoup(html_content, "html.parser")
        ...
        # Find the location-specific ticket section
        showtimes = cls._extract_location_showtimes(soup)
    ...
    @classmethod
    def _extract_location_showtimes(cls, soup: BeautifulSoup) -> List[ComedyWorksDowntownShowtime]:
        """Extract showtimes from this location's section only."""
        location_title = soup.select_one(f"p.club-title.{cls.location_css_class}")

        if location_title:
            ticket_loc = location_title.find_parent("div", class_="ticket-location")
            if ticket_loc:
