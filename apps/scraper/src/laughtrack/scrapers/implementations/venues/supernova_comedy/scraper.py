"""
SuperNova Comedy scraper implementation.

SuperNova Comedy (supernovacomedy.com) lists upcoming shows on their
Tixr group page (tixr.com/groups/supernova). The scraper:
...
Background: The generic Tixr scraper gets HTTP 403 from DataDome. Even Playwright
fallback is blocked. Only bare curl_cffi via TixrClient._fetch_tixr_page() bypasses
DataDome, following the Laugh Factory Covina pattern.
"""
...
from .extractor import SuperNovaComedyEventExtractor
from .data import SuperNovaComedyPageData
from .transformer import SuperNovaComedyEventTransformer


class SuperNovaComedyScraper(BaseScraper):
    """
    Scraper for SuperNova Comedy club.

    Extracts Tixr event URLs from the SuperNova Comedy Tixr group page, then
    ...
    key = "supernova_comedy"

    def __init__(self, club: Club, **kwargs):
        super().__init__(club, **kwargs)
        self.transformation_pipeline.register_transformer(SuperNovaComedyEventTransformer(club))
    ...
    async def get_data(self, url: str) -> Optional[SuperNovaComedyPageData]:
        """
        Fetch the SuperNova Comedy Tixr group page and extract TixrEvent objects.

        Args:
            url: The Tixr group page URL (tixr.com/groups/supernova)

        Returns:
            SuperNovaComedyPageData containing TixrEvent objects, or None if no events found
        """
        try:
            ...
            all_tixr_urls = SuperNovaComedyEventExtractor.extract_tixr_urls(html_content)
            ...
            return SuperNovaComedyPageData(event_list=tixr_events, tixr_urls=tixr_urls)
