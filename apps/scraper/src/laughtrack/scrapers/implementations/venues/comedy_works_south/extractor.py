...
Reuses the same HTML parsing logic as the Downtown scraper but filters
for the South location section (CSS class 'club-south') instead of Downtown.
"""
...
class ComedyWorksSouthExtractor:
    """Utility class for extracting Comedy Works South event data from HTML."""

    @staticmethod
    def extract_comedian_slugs(html_content: str) -> List[str]:
        """
        Parse the events list page and return unique comedian slugs for South shows.

        Args:
            html_content: HTML from /events?south=1

        Returns:
            Deduplicated list of comedian URL slugs
        """
    ...
    def extract_events_from_detail(html_content: str, slug: str) -> List[ComedyWorksDowntownEvent]:
        """
        Parse a comedian detail page and extract all South showtimes.
        ...
        Returns:
            List of ComedyWorksDowntownEvent, one per South showtime
        """
        soup = BeautifulSoup(html_content, "html.parser")
        ...
        # Find the South ticket-location section
        showtimes = ComedyWorksSouthExtractor._extract_south_showtimes(soup)
    ...
    @staticmethod
    def _extract_south_showtimes(soup: BeautifulSoup) -> List[ComedyWorksDowntownShowtime]:
        """Extract showtimes from the South location section only."""
        # Find the South club-title to scope to the right location
        south_title = soup.select_one("p.club-title.club-south")

        if south_title:
            # Navigate up to ticket-location container, then find its sibling show-times
            ticket_loc = south_title.find_parent("div", class_="ticket-location")
            if ticket_loc:
        ...
        for item in items:
            st = ComedyWorksSouthExtractor._parse_showtime(item)
            if st:
                showtimes.append(st)
