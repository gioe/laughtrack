...
class BroadwayComedyClubScraper(BaseScraper):
    ...
    key = "broadway"
    tessera_client_cls = BroadwayTesseraClient

    def __init__(self, club: Club, **kwargs):
        super().__init__(club, **kwargs)
        self.transformation_pipeline.register_transformer(BroadwayEventTransformer(club))
        self.tessera_client = self.tessera_client_cls(club, proxy_pool=self.proxy_pool)
    ...
    async def get_data(self, url: str) -> Optional[BroadwayEventData]:
        ...
        try:
            ...
            Logger.info(f"{self._log_prefix}: Processing URL: {normalized_url}", self.logger_context)
            ...
            Logger.info(
                f"{self._log_prefix}: Post-enrichment: {len(event_list)} events, {enriched_count} with ticket data",
                self.logger_context,
            )
    ...
    async def _enrich_events_with_tickets(self, events: List[BroadwayEvent]) -> List[BroadwayEvent]:
        ...
        Tessera events with no ticket data are filtered out by TesseraTicketBatchEnricher.enrich()
        — no per-scraper filter is needed here.
        """
        try:
            event_ids = self._select_tessera_event_ids(events)
            if not event_ids:
                Logger.info(f"{self._log_prefix}: No Tessera events found to enrich", self.logger_context)
                return events
            ...
            if self._tickets is None:
                ...
                Logger.info(f"{self._log_prefix}: Ticket enrichment disabled for this scraper", self.logger_context)
                return events
            ...
            Logger.info(
                f"{self._log_prefix}: Successfully enriched {len([e for e in enriched if getattr(e, '_ticket_data', None)])} events with ticket data",
                self.logger_context,
            )
            return enriched

        except Exception as e:
            Logger.error(f"{self._log_prefix}: Error enriching events with tickets: {str(e)}", self.logger_context)
            return events
    ...
    async def close(self):
        ...
        try:
            ...
            Logger.error(f"{self._log_prefix}: Error closing HTTP session: {e}", self.logger_context)
        ...
        try:
            ...
            Logger.error(f"{self._log_prefix}: Error closing Tessera client: {e}", self.logger_context)
