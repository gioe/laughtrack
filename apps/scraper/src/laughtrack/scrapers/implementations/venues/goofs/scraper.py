...
class GoofsComedyClubScraper(BaseScraper):
    ...
    async def get_data(self, url: str) -> Optional[GoofsPageData]:
        ...
        try:
            ...
            Logger.info(f"{self._log_prefix}: fetching shows page: {normalized_url}", self.logger_context)

            html = await self.fetch_html(normalized_url)
            if not html:
                Logger.warn(f"{self._log_prefix}: empty HTML from shows page", self.logger_context)
                return None

            events = GoofsEventExtractor.extract_shows(html)
            if not events:
                Logger.warn(f"{self._log_prefix}: no shows extracted from page", self.logger_context)
                return None

            Logger.info(f"{self._log_prefix}: extracted {len(events)} shows", self.logger_context)
            return GoofsPageData(event_list=events)

        except Exception as e:
            Logger.error(f"{self._log_prefix}: error scraping page {url}: {e}", self.logger_context)
            return None
