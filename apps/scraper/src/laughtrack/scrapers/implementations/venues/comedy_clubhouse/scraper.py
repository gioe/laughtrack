...
class ComedyClubhouseScraper(BaseScraper):
    ...
    async def get_data(self, url: str) -> Optional[ComedyClubhousePageData]:
        ...
        try:
            ...
            if not html:
                Logger.warn(
                    f"ComedyClubhouseScraper: empty response for {url}",
                    self.logger_context,
                )
                return None
            ...
            if not events:
                Logger.info(
                    f"ComedyClubhouseScraper: no events found on {url}",
                    self.logger_context,
                )
                return None

            Logger.info(
                f"ComedyClubhouseScraper: extracted {len(events)} events from {url}",
                self.logger_context,
            )
            ...
            Logger.error(
                f"ComedyClubhouseScraper: error fetching {url}: {e}",
                self.logger_context,
            )
            return None
