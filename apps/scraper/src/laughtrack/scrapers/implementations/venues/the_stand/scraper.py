...
class TheStandNYCScraper(BaseScraper):
    ...
    async def discover_urls(self) -> List[str]:
        ...
        try:
            ...
            Logger.info(
                f"{self._log_prefix}: Discovered {len(all_urls)} URLs across {months_processed} months",
                self.logger_context,
            )
            return all_urls

        except Exception as e:
            Logger.error(f"{self._log_prefix}: URL discovery failed: {e}", self.logger_context)
    ...
    async def _discover_month_horizontal_urls(self, start_url: str) -> List[str]:
        ...
        while current_url:
            ...
            try:
                ...
                Logger.error(f"{self._log_prefix}: Error processing horizontal page: {e}", self.logger_context)
                break

        Logger.info(f"{self._log_prefix}: Found {len(month_urls)} URLs in month via horizontal pagination", self.logger_context)
        return month_urls
    ...
    async def _get_next_month_url(self, current_url: str) -> Optional[str]:
        ...
        try:
            ...
            Logger.error(f"{self._log_prefix}: Error finding next month URL from {current_url}: {e}", self.logger_context)
            return None
    ...
    async def get_data(self, url: str) -> Optional[TheStandPageData]:
        ...
        try:
            ...
            if not tixr_events:
                Logger.info(f"{self._log_prefix}: No TixrEvents returned from {len(tixr_urls)} URLs on {url}", self.logger_context)
                return None
