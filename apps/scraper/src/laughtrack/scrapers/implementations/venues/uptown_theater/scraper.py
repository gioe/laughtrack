...
class UptownTheaterScraper(BaseScraper):
    ...
    async def get_data(self, url: str) -> Optional[UptownTheaterPageData]:
        ...
        try:
            ...
            if not html:
                Logger.warn(f"Failed to fetch events listing page: {listing_url}", self.logger_context)
                return None

            event_urls = self._extract_event_urls(html, listing_url)
            if not event_urls:
                Logger.warn(f"No event URLs found on listing page: {listing_url}", self.logger_context)
                return None

            Logger.info(f"Found {len(event_urls)} event URLs on listing page", self.logger_context)
            ...
            if not event_list:
                Logger.warn("No events extracted from any event page", self.logger_context)
                return None

            return UptownTheaterPageData(event_list)

        except Exception as e:
            Logger.error(f"Error scraping Uptown Theater listing: {e}", self.logger_context)
            return None
    ...
    def _extract_event_urls(self, html: str, listing_url: str) -> List[str]:
        ...
        try:
            ...
            Logger.error(f"Error extracting event URLs from listing page: {e}", self.logger_context)
            return []
    ...
    async def _scrape_event_page(self, event_url: str) -> Optional[List[JsonLdEvent]]:
        ...
        try:
            ...
            if not html:
                Logger.warn(f"Failed to fetch event page: {normalized_url}", self.logger_context)
                return None

            events = EventExtractor.extract_events(html)
            if not events:
                Logger.warn(f"No JSON-LD events found on event page: {normalized_url}", self.logger_context)
                return None
