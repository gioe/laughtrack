...
from .data import ComicStripEdmontonPageData
from .extractor import ComicStripEdmontonExtractor
from .transformer import ComicStripEdmontonTransformer
...
class ComicStripEdmontonScraper(BaseScraper):
    ...
    def __init__(self, club: Club, **kwargs):
        ...
        self.transformation_pipeline.register_transformer(ComicStripEdmontonTransformer(club))
    ...
    async def get_data(self, target: ScrapingTarget) -> Optional[ComicStripEdmontonPageData]:
        ...
        events = ComicStripEdmontonExtractor.extract_events(html, source_url=url)
        if not events:
            Logger.warn(f"{self._log_prefix}: no Edmonton Webflow event cards found: {url}", self.logger_context)
            return None

        return ComicStripEdmontonPageData(event_list=events)
