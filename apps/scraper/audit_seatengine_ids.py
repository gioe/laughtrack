"""
One-off audit: verify all SeatEngine v1 seatengine_id mappings against the API.
...
async def run_audit(fix_sql: bool = False):
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, name, seatengine_id FROM clubs WHERE scraper = 'seatengine' ORDER BY id"
            )
            clubs = cur.fetchall()

    token = os.environ["SEATENGINE_AUTH_TOKEN"]

    print(f"Auditing {len(clubs)} SeatEngine v1 clubs...\n")
...
def main():
    parser = argparse.ArgumentParser(description="Audit SeatEngine v1 ID mappings")
