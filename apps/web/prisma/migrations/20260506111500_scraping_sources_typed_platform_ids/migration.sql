...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_seatengine_id_unique
    ON scraping_sources (seatengine_id)
    WHERE platform = 'seatengine'::"ScrapingPlatform"
      AND enabled = true
      AND seatengine_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_eventbrite_id_unique
    ON scraping_sources (eventbrite_id)
    WHERE platform = 'eventbrite'::"ScrapingPlatform"
      AND enabled = true
      AND eventbrite_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_ticketmaster_id_unique
    ON scraping_sources (ticketmaster_id)
    WHERE platform = 'ticketmaster'::"ScrapingPlatform"
      AND enabled = true
      AND ticketmaster_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_wix_event_id_unique
    ON scraping_sources (wix_event_id)
    WHERE platform = 'wix_events'::"ScrapingPlatform"
      AND enabled = true
      AND wix_event_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_ovationtix_id_unique
    ON scraping_sources (ovationtix_id)
    WHERE platform = 'ovationtix'::"ScrapingPlatform"
      AND enabled = true
      AND ovationtix_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_squadup_id_unique
    ON scraping_sources (squadup_id)
    WHERE platform = 'squadup'::"ScrapingPlatform"
      AND enabled = true
      AND squadup_id IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS scraping_sources_seatengine_v3_id_unique
