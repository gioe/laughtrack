-- Hide duplicate tour_dates stub for Stress Factory Bridgeport (club 2465).
--
-- Verification on 2026-05-15:
--   * Duplicate club 2465 has 0 shows and no tagged/subscription/email/
--     production-company dependent rows.
--   * Canonical club 46 ("Stress Factory Bridgeport", Bridgeport CT,
--     chain 5) is visible, active, and has enabled SeatEngine Classic source
--     631 (source_url 'bridgeport.stressfactory.com/events').
--   * A targeted scrape of canonical club 46 with source 631 extracted and
--     persisted 40 shows from https://bridgeport.stressfactory.com/events.
--   * No Bridgeport Stress Factory aliases currently exist in club_aliases.

CREATE TEMP TABLE _stress_factory_bridgeport_duplicate (
...
INSERT INTO _stress_factory_bridgeport_duplicate (
    duplicate_id,
    duplicate_name,
    duplicate_city,
    duplicate_state,
...
VALUES (
    2465,
    'The Stress Factory comedy club, Bridgeport',
    'Bridgeport',
    'CT',
    1473,
    46,
...
BEGIN
    SELECT COUNT(*)
    INTO bad_count
    FROM _stress_factory_bridgeport_duplicate m
    ...
    LEFT JOIN scraping_sources canonical_source
       ...
       AND canonical_source.source_url = 'bridgeport.stressfactory.com/events'
    ...
    IF bad_count > 0 THEN
        RAISE EXCEPTION 'Cannot hide duplicate Stress Factory Bridgeport: expected duplicate/canonical rows are missing or changed';
    END IF;
    ...
    IF bad_count > 0 THEN
        RAISE EXCEPTION 'Cannot hide duplicate Stress Factory Bridgeport: duplicate club has dependent rows';
    END IF;
END $$;
...
SELECT
    ...
    '20260515062500: duplicate tour_dates stub ' || m.duplicate_id,
    TRUE
FROM _stress_factory_bridgeport_duplicate m
CROSS JOIN (
    VALUES
