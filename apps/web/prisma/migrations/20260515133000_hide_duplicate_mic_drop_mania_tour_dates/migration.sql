-- Hide duplicate tour_dates stub for "Mic Drop Mania" (club 2484).
--
-- Verification on 2026-05-15:
--   * Duplicate club 2484 has 0 shows and no tagged/subscription/email/
--     production-company dependent rows.
--   * Canonical club 120 ("Mic Drop Comedy Chandler") is visible, active, and
--     has an enabled SeatEngine source (id 96, scraper_key 'seatengine',
--     seatengine_id 493).
--   * The canonical SeatEngine source scraped successfully on 2026-05-15 with
--     240 shows from venue id 493.
--   * The official site identifies the venue as Mic Drop Comedy Chandler at
--     51 E. Boston St., Chandler AZ 85225, and the Tisha Campbell "DAMN GINA"
--     event from the tour_dates stub is present at /shows/352468.
...
VALUES
    (2484, 'Mic Drop Mania', 'Chandler', 'AZ', 1492, 120, 'Mic Drop Comedy Chandler', 96, 'Mic Drop Mania');
...
BEGIN
    ...
    LEFT JOIN scraping_sources canonical_source
       ...
       AND canonical_source.seatengine_id = 493
    ...
    IF bad_count > 0 THEN
        RAISE EXCEPTION 'Cannot hide duplicate Mic Drop Mania club 2484: expected duplicate/canonical rows are missing or changed';
    END IF;
    ...
    IF bad_count > 0 THEN
        RAISE EXCEPTION 'Cannot hide duplicate Mic Drop Mania club 2484: dependent rows exist';
    END IF;
END $$;
...
SELECT
    ...
    '20260515133000: duplicate tour_dates stub ' || duplicate_id,
    TRUE
