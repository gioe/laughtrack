...
function groupRuns(runs: AdminPipelineRun[]) {
    const groups = new Map<string, AdminPipelineRun[]>();
    for (const run of runs) {
        const current = groups.get(run.pipelineKey) ?? [];
        current.push(run);
        groups.set(run.pipelineKey, current);
    }

    return Array.from(groups.entries())
        .map(([pipelineKey, groupedRuns]) => {
            const sortedRuns = [...groupedRuns].sort(
                (a, b) =>
                    new Date(b.exportedAt).getTime() -
                    new Date(a.exportedAt).getTime(),
            );
            return {
                pipelineKey,
                pipelineName: sortedRuns[0]?.pipelineName ?? pipelineKey,
        ...
        .sort(
            (a, b) =>
                new Date(b.latestRun?.exportedAt ?? 0).getTime() -
                new Date(a.latestRun?.exportedAt ?? 0).getTime(),
        );
}
...
export default function AdminPipelineRunsTable({
    ...
    const [openPipelineKeys, setOpenPipelineKeys] = useState<Set<string>>(
        new Set(),
    );
    const [openRunIds, setOpenRunIds] = useState<Set<number>>(new Set());

    function togglePipeline(pipelineKey: string) {
        setOpenPipelineKeys((current) => {
...
function PipelineRunAttempts({
    runs,
    openRunIds,
    toggleRun,
}: {
    runs: AdminPipelineRun[];
    openRunIds: Set<number>;
...
function PipelineRunDetails({ run }: { run: AdminPipelineRun }) {
    const isGithubRun = isGithubActionsRun(run);
    return (
        <div className="grid gap-4 font-dmSans text-body md:grid-cols-3">
            <dl className="grid gap-2">
                <DetailItem label="Run key" value={run.runKey} />
                <DetailItem label="Source" value={run.source ?? "scraper"} />
                <DetailItem
                    label="Exported"
                    value={formatDateTime(run.exportedAt)}
                />
            </dl>
            {isGithubRun ? (
                <dl className="grid gap-2">
                    <DetailItem
                        label="Workflow"
                        value={run.workflowName ?? run.pipelineName}
                    />
                    <DetailItem
                        label="Title"
                    ...
                    <DetailItem
                        label="Event"
                        value={run.event ?? "Not recorded"}
                    />
                    <DetailItem
                        label="Actor"
                        value={run.actor ?? "Not recorded"}
                    />
                    <DetailItem
                        label="Run number"
                        value={run.runNumber ?? "Not recorded"}
                    />
                    <DetailItem
                        label="Attempt"
                        value={run.runAttempt ?? "Not recorded"}
                    />
                    <DetailItem label="Ref" value={run.ref ?? "Not recorded"} />
                    <DetailItem
                        label="SHA"
                        value={shortSha(run.sha) ?? "Not recorded"}
                    />
                </dl>
            ) : (
                <dl className="grid gap-2">
