...
import { generateValidUrl, generateValidUrls } from "../../../util/primatives/urlUtil";
...
export class PageManager {
  ...
  getAllLinksOnPage = async (url: string, locator: playwright.Locator): Promise<string[]> => {
    return this.getAllHrefs(locator).then((hrefs: string[]) => generateValidUrls(url, hrefs))
  }

  getLinksAcrossPages = async (page: Page,
    nextSelector: string,
    allSelector: string,
    allPageLinks?: string[],
    allScrapedLinks?: string[]): Promise<string[]> => {
    ...
    return this.getAllLinksOnPage(page.url(), linkLocator)
      ...
      .then((page: Page) => this.getLinksAcrossPages(page, nextSelector, allSelector, pageLinks, scrapedLinks))
      .catch((error) => {
        console.warn(error)
  ...
  select = async (page: Page,
    selector: string,
    option: string): Promise<Page> => {
    return page.selectOption(selector, option)
      .then(() => page)
  }

  clickPageButton = async (page: Page,
    locator: Locator): Promise<Page> => {
  ...
  getHref = async (locator: playwright.Locator): Promise<string> => {
    ...
    return locator.getAttribute('href', { timeout: 1000 }).then(href => href ? removeBadWhiteSpace(href) : "")
  }

  getValue = async (locator: playwright.Locator): Promise<string> => {
    await locator.waitFor({ state: 'attached', timeout: 1000 });
    return locator.getAttribute('value', { timeout: 1000 }).then(value => value ? removeBadWhiteSpace(value) : "")
  }

  getTextContent = async (locator: playwright.Locator): Promise<string> => {
    await locator.waitFor({ state: 'attached', timeout: 1000 });
    return locator.textContent({ timeout: 1000 }).then((text: string | null) => text ? removeBadWhiteSpace(text) : "")
  }
