...
export async function runScrapingJob(clubIds?: string[], headless?: boolean): Promise<string> {
...
const runScraper = async (
    ...
    const shouldShowBrowser = headless ?? false
    return playwright.chromium
        .launch({ headless: !shouldShowBrowser })
    ...
    if (instance.comedians.length > 0) {
        return db.comedians.addAll(instance.comedians)
            ...
            .then((comedianIds: { id: number }[]) => {
                ...
                return db.lineups.addAll(lineupItems);
            })
    }
