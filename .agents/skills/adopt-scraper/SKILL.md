...
Record:
- **Website URL** (use `https://` — see AGENTS.md rule on HTTPS)
...
(see AGENTS.md rule), not plain `requests`/`urllib`. If Playwright fallback
...
`npx prisma migrate deploy` cannot run locally (see AGENTS.md — local `.env.local` points to
...
- Is the API returning data? (test the endpoint directly — see AGENTS.md on using
