...
**1b. AGENTS.md convention scan:**

```bash
ls AGENTS.md 2>/dev/null && echo "exists" || echo "not found"
```

If AGENTS.md exists, scan for any section whose heading contains the word "convention" (case-insensitive). Extract bullet points (lines beginning with `-` or `*`). If no matching section is found, or if the section contains only a pointer line (e.g., "Run `tusk conventions list`..."), there is nothing to migrate.
...
Compare against AGENTS.md bullets to identify which bullets are not already in the DB. These are migration candidates.
...
|---------|---------------|---------------|
| AGENTS.md convention bullets not in DB | Migrate all bullets and replace section with pointer line | DB-stored conventions are queryable by topic; AGENTS.md bullets are not |
...
Then Edit `AGENTS.md` to replace the "Key Conventions" section body with `Run \`tusk conventions list\` to see project conventions.` (leave the heading in place).
...
If `project_type` was changed in Step 5 (set, cleared, or replaced), re-run the `applies_to_project_types` filter so `.agents/skills/` matches the new value:
