...
- **[Executed — TASK-3646, 2026-07-09]** 9 import sites codemodded to core paths, `domain/` deleted, anti-reintroduction import-linter contract added (sources enumerate the 9 laughtrack subpackages — a root-package source errors once the module exists again). Convention #327 recorded for new venue entities. Gotcha hit: deleting the facade changed test-collection import order, letting the `tests/core/entities/conftest.py` setdefault MagicMock stub for ComedianUtils leak into headliner tests — fixed by pre-importing the real module (see convention #41). The old contract's transitive core-entities→infrastructure guarantee is NOT restored (`laughtrack.core.entities` is a namespace package import-linter rejects as a source) — tracked as TASK-3695.
...
- **[Closed wont_do — TASK-3661, 2026-07-09]** Premise stale: the regression no longer manifests. Only two suites still mount `HostedView` (AppShellViewTests, HostedViewDumpTreeTests — the rest migrated off it around TASK-2021), and a local iOS 26.1 run passed 14/14 with a wiring probe confirming intact accessibility; the watcher's latest test-executing run on main was green (its same-day reds were fastlane `showBuildSettings` startup timeouts, not test failures). A guard would be permanently inert. Follow-up TASK-3694 stabilizes the watcher flake and evaluates dropping the 18.x pin; a complete guard implementation is preserved on task 3661's decision atom #139 if the regression returns.
