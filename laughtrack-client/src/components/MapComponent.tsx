...
interface MapComponentInterface {
  searchResults: ComedianInterface[]
}
...
const MapComponent: React.FC<MapComponentInterface> = ({
  ...
  const clubs = searchResults.flatMap((comedian: ComedianInterface) => comedian.schedule?.shows ? comedian.schedule.shows : [])
    .map((show: ShowInterface) => show.club)

  const uniqueClubs = [...new Set(clubs)]

  const coordinates = uniqueClubs.map((club: ClubInterface) => {
