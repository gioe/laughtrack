...
3. From the repository root, run exactly one of:
   ...
   ```bash
   apps/scraper/.venv/bin/python .claude/skills/send-test-push/scripts/send_test_push.py --email user@example.com
   apps/scraper/.venv/bin/python .claude/skills/send-test-push/scripts/send_test_push.py --user-id <user-id>
   ```

   Optional copy overrides:

   ```bash
   apps/scraper/.venv/bin/python .claude/skills/send-test-push/scripts/send_test_push.py \
