...
# ── Auto-generated review-noise files ─────────────────────────────────
#
# Files regenerated by package managers or API client generators and not
# hand-authored. Used by:
#   - tusk-merge.py — auto-resolve stash-pop conflicts in lockfiles by
#     preferring the WIP version.
#   - tusk-review-diff-range.py — subtract generated-section lines from the
#     diff_lines_meaningful count so the /review-commits routing threshold
#     tracks the size of human-readable changes (issue #761).
#
# Lockfiles are matched by basename, so the same path inside any subdirectory
# counts. Generated clients are matched by normalized repo-relative paths so
# hand-authored Swift/Kotlin outside those generated directories still count.
...
GENERATED_CLIENT_PATH_PREFIXES = (
    "android/core/network/src/main/kotlin/app/laughtrack/android/core/network/generated/",
    "ios/Sources/LaughTrackAPIClient/GeneratedSources/",
)


def is_generated_review_noise_path(path: str) -> bool:
    """Return True when *path* is generated noise for review-size routing."""
    normalized = path.replace("\\", "/").strip().strip('"')
    if os.path.basename(normalized) in GENERATED_LOCKFILES:
        return True
    return any(normalized.startswith(prefix) for prefix in GENERATED_CLIENT_PATH_PREFIXES)


def filter_generated_review_noise_diff_sections(diff_out: str) -> str:
    """Return *diff_out* with every per-file section for an auto-generated
    review-noise file removed (header line included).
    ...
    introducing header, and drops the section entirely when its path matches
    :func:`is_generated_review_noise_path`.
    ...
    Used to compute ``diff_lines_meaningful`` without altering the legacy
    ``diff_lines`` count.
    """
    if not diff_out:
        return diff_out
    ...
    for line in diff_out.splitlines(keepends=True):
        if line.startswith("diff --git "):
            ...
            if idx != -1:
                ...
                skip_current = is_generated_review_noise_path(b_path)
            else:
                skip_current = False
        if not skip_current:
...
def filter_lockfile_diff_sections(diff_out: str) -> str:
    """Backward-compatible alias for generated review-noise filtering."""
    return filter_generated_review_noise_diff_sections(diff_out)
