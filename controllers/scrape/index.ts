...
export const scrapeClubs = async (id: number[], headless?: boolean) => {
    ...
    await clubController
        ...
        .then((scrapingOutput: ScrapingOutput[][]) =>
            flattenArrayList(scrapingOutput),
        )
        .then((scrapingOutput: ScrapingOutput[]) => storeOutput(scrapingOutput))
        .then(() =>
            console.log(
                `Finished in ${(new Date().getTime() - startDate.getTime()) / 1000} seconds`,
            ),
        );
};
