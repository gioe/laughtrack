...
class ProfileRepository
    ...
    constructor(
        ...
        fun buildSignInUrl(provider: ProfileAuthProvider): String = accountService.buildSignInUrl(provider)
        ...
        suspend fun saveLocation(
            ...
            val next =
                localPreferences.preferences.first().copy(
                    zipCode = zipCode,
                    nearbyDistanceMiles = distanceMiles,
                )
            localPreferences.save(next)
            val synced =
                settingsService.updateLocation(
                ...
                settingsService.updateNotifications(
                    ProfileNotificationUpdate(
                        emailShowNotifications =
                            if (next.emailShowNotifications != previous.emailShowNotifications) {
                                next.emailShowNotifications
                            } else {
                                null
                            },
                        pushShowNotifications =
                            if (next.pushShowNotifications != previous.pushShowNotifications) {
...
@Singleton
class AuthSessionProfileAccountService
    @Inject
    constructor(
        ...
        override suspend fun getMe(): Result<MeData> = authSessionManager.getMe().map { it.data }
...
@Singleton
class NetworkProfileSettingsService
    @Inject
    constructor(
...
@Singleton
class DataStoreProfileLocalPreferences
    @Inject
    constructor(
        ...
        override val preferences: Flow<ProfilePreferences> =
            dataStore.data.map { preferences ->
                ProfilePreferences(
                    zipCode = preferences[Keys.zipCode],
                    nearbyDistanceMiles =
                        preferences[Keys.nearbyDistanceMiles]
