...
@Singleton
class FavoriteOfflineQueue @Inject constructor(
    private val workManager: WorkManager,
) {
    fun enqueue(entity: FavoriteEntity, id: String, isFavorite: Boolean) {
        val request = OneTimeWorkRequestBuilder<FavoriteReplayWorker>()
            .setConstraints(
                Constraints.Builder()
    ...
    companion object {
        ...
        fun uniqueName(entity: FavoriteEntity, id: String): String =
            "favorite-${entity.name.lowercase()}-$id"
    }
}

@HiltWorker
class FavoriteReplayWorker @AssistedInject constructor(
    @Assisted appContext: Context,
    @Assisted workerParameters: WorkerParameters,
    private val favoritesApi: FavoritesApi,
) : CoroutineWorker(appContext, workerParameters) {
    override suspend fun doWork(): Result {
        val entity = inputData.getString(FavoriteOfflineQueue.KEY_ENTITY)
        ...
        val response = runCatching {
            when (entity) {
                FavoriteEntity.COMEDIAN -> if (isFavorite) {
